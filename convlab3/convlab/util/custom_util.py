...
def move_finished_training(dir_in, dir_to):
    ...
    dest = os.path.join(dir_to, os.path.basename(dir_in))
    try:
        shutil.copytree(dir_in, dest)
        shutil.rmtree(dir_in)
    except Exception as e:
        logging.warning(f"Could not fully move experiment to finished_experiments: {e}")
        logging.info(f"Experiment remains available at: {dir_in}")
    else:
...
def _get_goal_class(goal_generator):
    try:
        module_name = goal_generator.__class__.__module__
        if module_name.startswith('convlab.task.simplemultiwoz21'):
            from convlab.policy.rule.simplemultiwoz21.policy_agenda_multiwoz import Goal
            return Goal
    except Exception:
        pass
...
def create_goals(goal_generator, num_goals, single_domains=False, allowed_domains=None):
    Goal = _get_goal_class(goal_generator)
