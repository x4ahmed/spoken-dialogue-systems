...
class PPO(Policy):
    ...
    def __init__(self, is_train=False, seed=0, vectorizer=None, load_path="", config_file='ppo_config.json', **kwargs):
    ...
    def est_adv(self, r, v, mask):
        ...
        # initialize tensors. Each has the same length as the batch size, i.e. episode length
	...
	#######################################################################################################
	## TODO Policy_TASK1 Generalized Advantage Estimation (GAE)
	## Here we compute the GAE used for policy update
	## Start of your code #################################################################################

        for t in reversed(range(batchsz)):
            # NOTE: pay attention that the iteration here goes from the end of the episode to the beginning
            # mask here indicates a end of trajectory
            # mask = 0 means the immediate reward is the real V(s) since it's end of trajectory.

            #######################################################################################################
            ## TODO 1a compute v_target, i.e. V(s_t)
            ## v_target is worked out by Bellman equation. This value will be treated as the target value of value network.
            ...
            #######################################################################################################
            ## TODO 1c compute A(s_t, a_t)
            ## formula: A(s, a) = delta(s_t) + gamma * lamda * A(s_t+1, a_t+1)
            ## NOTE: in the code we use variable tau as lambda, but original paper uses symbol lambda.

            A_sa[t] = delta[t] + self.gamma * self.tau * mask[t] * prev_A_sa
            #######################################################################################################

            # update previous target for next t in the iteration
    ...
    def update(self, epoch, batchsz, s, a, r, mask, action_mask):
        #######################################################################################################
	## TODO Policy_TASK2 Policy update
	## Here use the GAE and PPO clip to update the policy
        ## NOTE: action mask is used to exclude non-permissible actions during action selection
	## Start of your code #################################################################################

        # compute mean training reward for logging
        mean_reward = r.float().mean().item()
        ...
        #######################################################################################################
        ## TODO 2a get log prob of pi_old(s, a) from self.policy (tip: check the policy class code to see which method to use)
        ## when interacting with env, pi_old(s, a) can be simply be saved by using the policy before the update to save time
        ...
        for i in range(self.update_round):
            ...
            for v_target_b, A_sa_b, s_b, a_b, log_pi_old_sa_b, action_mask_b in zip(v_target_shuf, A_sa_shuf, s_shuf, a_shuf,
                ...
                #######################################################################################################
                ## TODO 2b Compute critic loss
                ## critic loss is the MSE of the value prediction and value target
                ...
                # Step 2: Update actor network

                self.policy_optim.zero_grad()

                #######################################################################################################
                ## TODO 2c Compute policy ratio
                ## NOTE: we use log probability instead of probability for stability of numerical operation
                ## Therefore  ratio = Pi(a|s) / Pi_old(a|s) = exp(log_Pi(a|s) - log_Pi_old(a|s)) 
                ...
                #######################################################################################################
                ## TODO 2d Compute PPO-clip objective
        ...
        return policy_loss, value_loss, mean_reward
    ...
    def load_from_pretrained(self, model_path=""):
        ...
        if os.path.exists(model_path):
            # Direct local file path (e.g., /path/to/best_mle.pol.mdl)
            logging.info(f"Loading local pretrained model from: {model_path}")
            self.load(model_path.replace('.pol.mdl', ''))
            return
        else:
