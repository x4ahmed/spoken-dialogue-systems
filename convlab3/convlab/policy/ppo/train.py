...
def update(env, policy, num_dialogues, epoch, process_num, seed=0, user_reward=False):
    ...
    policy_loss, value_loss, mean_reward = policy.update(epoch, batchsz_real, s, a, r, mask, action_mask)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_eval_dials", type=bool, default=True,
    ...
    ppo_config_file = conf.get('model', {}).get('ppo_config', 'ppo_config.json')
    ...
    eval_dict = eval_policy(conf, policy_sys, env, sess,
                            save_eval, log_save_path, allowed_domains = ['restaurant', 'hotel'])
    ...
    for i in range(conf['model']['epoch']):
        ...
        policy_loss, value_loss, mean_reward = update(env, policy_sys, conf['model']['num_train_dialogues'], idx, conf['model']['process_num'], seed=seed, user_reward=use_user_reward)

        # Log training metrics to TensorBoard every epoch
        tb_writer.add_scalar("policy_loss", policy_loss, idx * conf['model']['num_train_dialogues'])
        tb_writer.add_scalar("value_loss", value_loss, idx * conf['model']['num_train_dialogues'])
        tb_writer.add_scalar("mean_train_reward", mean_reward, idx * conf['model']['num_train_dialogues'])
        logging.info(f"Epoch {idx} | policy_loss: {policy_loss:.4f} | value_loss: {value_loss:.4f} | mean_reward: {mean_reward:.4f}")
        ...
        if idx % conf['model']['eval_frequency'] == 0 and idx != 0:
            ...
            eval_dict = eval_policy(
                conf, policy_sys, env, sess, save_eval, log_save_path, allowed_domains = ['restaurant', 'hotel'])
