...
def get_goal_generator(dataset_name='multiwoz21'):
    if dataset_name == 'simplemultiwoz21':
        from convlab.task.simplemultiwoz21.goal_generator import GoalGenerator
    else:
...
def evaluate(config_path, model_name, verbose=False, model_path="", goals_from_data=False, dialogues=500):
    ...
    dataset_name = getattr(policy_sys.vector, 'dataset_name', 'multiwoz21')
    goal_generator = get_goal_generator(dataset_name)
    if goals_from_data:
        logging.info("read goals from dataset...")
        goals = data_goals(dialogues, dataset=dataset_name, dial_ids_order=0)
    else:
