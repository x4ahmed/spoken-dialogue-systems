...
def read_data(exp_dir, tb_dir, map_file):
    ...
    train_data = {}
    for m in f_map:
        data[m["legend"]], train_data[m["legend"]] = read_dir(exp_dir, tb_dir, m["dir"])
...
def read_dir(exp_dir, tb_dir, method_dir):
    dfs = []
    train_dfs = []
    for dir_name in tqdm(glob(os.path.join(exp_dir, method_dir, tb_dir)), ascii=True, desc=method_dir):
        df, train_df = read_tb_data(dir_name)
        dfs.append(df)
        train_dfs.append(train_df)
    return dfs, train_dfs
...
def read_tb_data(in_path):
    ...
    # Separate eval metrics from training metrics (they have different # of data points)
    train_metrics = ['policy_loss', 'value_loss', 'mean_train_reward']
    eval_keys = [k for k in keys if k not in train_metrics]
    train_keys = [k for k in keys if k in train_metrics]

    # Read eval metrics into a DataFrame
    df = pd.DataFrame()
    for key in eval_keys:
        scalar_events = event_data.Scalars(key)
        step_nums = [e.step for e in scalar_events]
        vals = [e.value for e in scalar_events]
        df[key] = vals
        df['steps'] = step_nums

    # Read training metrics into a separate DataFrame
...
def plot(data, out_file, plot_type="complete_rate", show_image=False, fill_between=0.3, max_dialogues=0, y_label='',
    ...
    plt.savefig(out_file + ".png", bbox_inches='tight', dpi=400, pad_inches=0)
...
if __name__ == "__main__":
    ...
    for plot_type in ["complete_rate", "success_rate", "success_rate_strict", 'turns', 'avg_return', 'avg_actions']:
        ...
        data, train_data = read_data(exp_dir=args.dir, tb_dir=args.tb_dir,
    ...
    # Plot training metrics (policy_loss, value_loss, mean_train_reward)
    train_label_dict = {"policy_loss": 'Policy Loss', "value_loss": 'Value Loss',
                         "mean_train_reward": 'Mean Training Reward'}

    for plot_type in ["policy_loss", "value_loss", "mean_train_reward"]:
        file_name, file_extension = os.path.splitext(args.out_file)
        fig_name = f"{file_name}_{plot_type}{file_extension}"
        plot(data=train_data,
             out_file=fig_name,
             plot_type=plot_type,
             fill_between=args.fill_between,
             max_dialogues=args.max_dialogues,
             y_label=train_label_dict[plot_type],
             fontsize=args.fontsize,
             figsize=args.figure_size,
             facecolor=args.figure_face_color)
