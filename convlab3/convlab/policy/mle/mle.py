...
DEFAULT_DIRECTORY = os.path.join(BASE_DIR, "models")
DEFAULT_ARCHIVE_FILE = os.path.join(DEFAULT_DIRECTORY, "mle_policy_multiwoz.zip")
DEFAULT_CONFIG_FILE = "configs/saved_config.json"  # Relative path to model_dir
...
class MLEAbstract(Policy):
    ...
    def load_from_pretrained(self, archive_file, model_file, filename):
        ...
        model_dir = os.path.join(model_file, 'save')
        ...
        policy_mdl = os.path.join(os.path.dirname(model_dir), filename + '_mle.pol.mdl')
...
class MLEPolicy(MLE):
    def __init__(self,
        ...
        # super().__init__()
        if model_file:
            with open(os.path.join(model_file, 'configs/config_saved.json'), 'r') as f:
                cfg = json.load(f)
            
            self.vector = VectorBinary(dataset_name=cfg['args']['dataset_name'], use_masking=cfg['args']['use_masking'])
            self.policy = MultiDiscretePolicy(self.vector.state_dim, cfg['config']['h_dim'], self.vector.da_dim).to(device=DEVICE)
            self.load_from_pretrained(archive_file, model_file, cfg['config']['load'])
