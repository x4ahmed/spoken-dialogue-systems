...
def train(args, train_dataset, features, model, tokenizer, processor, continue_from_global_step=0):
    ...
    for epoch_idx, _ in enumerate(train_iterator):
        ...
        if args.local_rank in [-1, 0] and epoch_steps > 0:
            ...
            logger.info("Epoch %d finished: global_step = %d, average loss = %f", epoch_idx + 1, global_step, avg_epoch_loss)
...
def load_and_cache_examples(args, model, tokenizer, processor, evaluate=False):
    ...
    cached_file = os.path.join(args.output_dir, 'cached_{}_{}_features'.format(args.task_name,
