...
def get_joint_slot_correctness(fp, class_types, label_maps,
    ...
    with open(fp) as f:
        ...
        for pred in preds:
            ...
            # if turn_pd_class != 0:
                # print(turn_gt_class, turn_pd_class)
        ...
        for ct in range(len(class_types)):
            # Default to 0.0 to prevent false inflation of metrics
        ...
        return np.asarray(total_correctness), np.asarray(val_correctness), [np.asarray(class_correctness[i]) for i in range(len(class_correctness))], np.asarray(pos_correctness), np.asarray(refer_correctness), confusion_matrix, c_tp, c_tn, c_fp, c_fn
